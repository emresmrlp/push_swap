...
/*   Updated: 2025/03/16 23:26:02 by ysumeral         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	tester(t_data *data)
{
	...
	{
		if (stack->target)
			ft_printf("STACK A (%d): A.M. (%d), INDEX (%d), TARGET (%d) cost (%d) cheapest? (%d)\n", stack->value, stack->above_median, stack->index, stack->target->value, stack->cost, stack->cheapest);
		else
	...
	{
		if (stack->target)
			ft_printf("STACK B (%d): A.M. (%d), INDEX (%d), TARGET (%d) cost (%d) cheapest? (%d)\n", stack->value, stack->above_median, stack->index, stack->target->value, stack->cost, stack->cheapest);
		else
